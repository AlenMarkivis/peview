...
(function () {
  ...
  function reveals() {
    ...
    var groups = ['.phil-row', '.ind-grid', '.help-grid', '.masonry__col', '.talent'];
  ...
  function philVideos() {
    ...
    // Matches the opacity transition on .phil-item__video; the clip keeps running
    // until the still has fully faded back in so the swap never snaps.
    var FADE_MS = 450;

    vids.forEach(function (v) {
      var orb = v.closest('.phil-item');
      if (!orb) return;
      var timer = null;
      var play = function () {
